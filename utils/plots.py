...
def detect_track(img, punkte):
    blau=[]
    gelb=[]
    for i in range(len(punkte)):    

        if punkte[i][2]==0:
            blau.append(punkte[i])
        if punkte[i][2]==1:
            gelb.append(punkte[i])

    for i in range(len(blau)-1):
        cv2.line(img,(blau[i][0],blau[i][1]),(blau[i+1][0],blau[i+1][1]),(255,0,0),10)
    for i in range(len(gelb)-1):
        cv2.line(img,(gelb[i][0],gelb[i][1]),(gelb[i+1][0],gelb[i+1][1]),(0,255,0),10)
